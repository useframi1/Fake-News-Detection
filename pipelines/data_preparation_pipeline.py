...
class DataPreparationPipeline:
    ...
    #     print(f"{'='*5} Generating adversarial examples... {'='*5}")
    ...
    #     victim = oa.Classifier(model, tokenizer)
    ...
    #     num_samples_to_augment = int(total_samples * augmentation_percentage)

    #     if equal_classes:
    #         samples_per_class = num_samples_to_augment // len(unique_classes)
    #     else:
    #         samples_per_class = (
    #             None  # Will augment proportionally based on class distribution
    #         )
    ...
    #     augmented_class_count = {cls: 0 for cls in unique_classes}

    #     for text, label in tqdm(zip(texts, labels), total=total_samples):
    #         try:
    #             # Pass the raw text (no tokenization)
    #             adv = attacker.attack(text, label, victim)
    #             if adv is not None:
    #                 # If equal_classes is True, only add to the class if we haven't reached the limit
    #                 if equal_classes:
    #                     if augmented_class_count[label] < samples_per_class:
    #                         adv_texts.append(adv)
    #                         adv_labels.append(label)
    #                         augmented_class_count[label] += 1
    #                 else:
    #                     adv_texts.append(adv)
    #                     adv_labels.append(label)

    #             # Stop once we have generated enough adversarial samples
    #             if len(adv_texts) >= num_samples_to_augment:
    #                 break
    ...
    #     adv_df = pd.DataFrame(
    #         {
    #             self.config["content_column"]: adv_texts,
    #             self.config["label_column"]: adv_labels,
    #         }
    #     )

    #     original_df = pd.concat([train_data["content"], train_data["label"]], axis=1)
    #     full_df = pd.concat([original_df, adv_df], ignore_index=True)

    #     # Shuffle the dataset
    #     full_df = full_df.sample(frac=1, random_state=42).reset_index(drop=True)

    #     train_data["content"] = full_df[self.config["content_column"]]
    #     train_data["label"] = full_df[self.config["label_column"]]

    #     return {"content": train_data["content"], "label": train_data["label"]}
